...
async function syncShiftToFirestore() {
    if (!currentShift.active || !shiftDocId) return;
    try {
        await updateDoc(doc(db, "activeShifts", shiftDocId), {
            sales: currentShift.sales,
            drops: currentShift.drops,
            cashierExpenses: currentShift.cashierExpenses
        });
    } catch(e) {}
}

async function loadShiftFromFirestore() {
    try {
        const snap = await getDocs(collection(db, "activeShifts"));
        if (!snap.empty) {
            const docData = snap.docs[0];
            const data = docData.data();
            shiftDocId = docData.id;
            currentShift = {
                active: true,
                ...
                sales: data.sales || 0,
                drops: data.drops || 0,
                cashierExpenses: data.cashierExpenses || 0,
                startTime: data.startTime
            };
            localStorage.setItem('activeShift', JSON.stringify(currentShift));
            localStorage.setItem('lastCashierName', JSON.stringify(data.cashierName));
            return true;
        }
        return false;
    } catch(e) {
...
async function enforceCloudSubscriptionLogic() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/codes/verify-token`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ client_name: GLOBAL_CLIENT_NAME, token: token })
        }).catch(() => null);
        ...
        if (!data.valid || data.remaining_days <= 0) {
            localStorage.removeItem('subscription_token');
            window.location.href = "subscribe.html";
            return;
        }

        updateSubscriptionUI(data.remaining_days, data.max_valid);

        const maxSidebarItem = document.getElementById('navLitePosBtn');
        if (maxSidebarItem) {
            maxSidebarItem.style.display = data.max_valid ? '' : 'none';
        }
...
function updateSubscriptionUI(proDays, maxValid) {
...
async function verifyPassword() {
    ...
    const valid = await verifyAdminPassword(input);
    if (valid) {
...
document.getElementById('startShiftBtn')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (snap.empty) {
            ...
            currentShift = { active: true, cashierName: name, startCash: startCash, sales: 0, drops: 0, cashierExpenses: 0, startTime: new Date().toISOString() };
            const docRef = await addDoc(collection(db, "activeShifts"), currentShift);
            shiftDocId = docRef.id;
            localStorage.setItem('activeShift', JSON.stringify(currentShift));
            localStorage.setItem('lastCashierName', JSON.stringify(name));
            startShiftModal.style.display = 'none';
    ...
    } catch (e) { alert("حدث خطأ في الاتصال."); } finally { btn.innerText = "استلام الوردية"; btn.disabled = false; }
});
...
document.getElementById('confirmDropBtn')?.addEventListener('click', async () => {
    ...
    if (!valid) return alert("كلمة مرور المدير غير صحيحة!");
...
document.getElementById('confirmEndShiftBtn')?.addEventListener('click', async () => {
    ...
    try {
        ...
        await addDoc(collection(db, "shifts"), { ...currentShift, endTime: new Date().toISOString(), actualCash: actual, expectedCash: expected, difference: diff, status: diff >= 0 ? 'زيادة' : 'عجز' });
        if (shiftDocId) { await deleteDoc(doc(db, "activeShifts", shiftDocId)); shiftDocId = null; }
        currentShift = { active: false, cashierName: "", startCash: 0, sales: 0, drops: 0, cashierExpenses: 0, startTime: null };
        localStorage.removeItem('activeShift');
        localStorage.removeItem('lastCashierName');
    ...
    } catch(e) { alert("خطأ في الحفظ"); } finally { btn.innerText = "إنهاء الوردية وبدء وردية جديدة"; btn.disabled = false; }
});
...
document.getElementById('barcodeInput')?.addEventListener('keypress', async (e) => {
    if (e.key === 'Enter') {
        ...
        if (local) { addToCart({...local}); e.target.value = ''; document.getElementById('searchDropdown')?.classList.remove('active'); return; }
        try {
            let found = false;
            ...
            if (found) { e.target.value = ''; document.getElementById('searchDropdown')?.classList.remove('active'); }
...
function renderCart() {
    ...
    cart.forEach((item, i) => {
        ...
        tbody.innerHTML += `<tr>
            ...
            <td>${it}</td>
            <td><button class="delete-btn" onclick="window.removeFromCart('${item.id}')">🗑</button></td>
        </tr>`;
    });
...
window.changeQty = (productId, delta) => {
    ...
    const newQty = cart[itemIndex].cartQty + delta;
    if (newQty > 0 && newQty <= cart[itemIndex].quantity) {
        cart[itemIndex].cartQty = newQty;
        renderCart();
    } else if (newQty <= 0) {
        window.removeFromCart(productId);
    } else {
        window.playSound('error');
        alert("الكمية لا تكفي");
    }
};
...
document.getElementById('checkoutBtn')?.addEventListener('click', async () => {
    ...
    const items = cart.map(i => {
        const itemTotal = roundMoney(i.sellPrice * i.cartQty);
        const itemCost = roundMoney(i.buyPrice * i.cartQty);
        ts = roundMoney(ts + itemTotal);
        tc = roundMoney(tc + itemCost);
...
async function loadStats() {
    ...
    let invoicesQuery;
    if (start) {
        invoicesQuery = query(collection(db, "invoices"), where("timestamp", ">=", start), where("timestamp", "<=", end));
    } else {
        invoicesQuery = collection(db, "invoices");
    }
    (await getDocs(invoicesQuery)).forEach(d => {
        const data = d.data();
    ...
    let expensesQuery;
    if (start) {
        expensesQuery = query(collection(db, "expenses"), where("date", ">=", start), where("date", "<=", end));
    } else {
        expensesQuery = collection(db, "expenses");
    }
    (await getDocs(expensesQuery)).forEach(d => {
        const data = d.data();
        te = roundMoney(te + (data.amount || 0));
    });
...
async function cleanupCamera(divId) {
    if (cameraInstances[divId]) {
        try {
            await cameraInstances[divId].stop();
            await cameraInstances[divId].clear();
        } catch(e) {}
        delete cameraInstances[divId];
    }
}
...
function setupCamera(btnId, divId, inputId, checkId, callback) {
    ...
    btn.addEventListener('click', async () => {
        ...
        if (open) {
            await cleanupCamera(divId);
            div.style.display = 'none';
            open = false;
            btn.innerHTML = '📷';
        } else {
            div.style.display = 'block';
            const qrCode = new window.Html5Qrcode(divId);
            cameraInstances[divId] = qrCode;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const firestoreShiftLoaded = await loadShiftFromFirestore();
    if (firestoreShiftLoaded) {
        document.getElementById('shiftInfoDisplay').innerText = `الكاشير: ${currentShift.cashierName} | العهدة: ${currentShift.startCash} ج`;
        startShiftModal.style.display = 'none';
        posSection.style.display = 'block';
        document.getElementById('barcodeInput')?.focus();
