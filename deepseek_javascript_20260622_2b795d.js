...
// تحديث التظليل في القائمة الجانبية
...
// ==========================================
// 6. التنقل بين شاشة البيع والإدارة
// ==========================================
...
// التنقل بين التبويبات مع التظليل
...
// ==========================================
// ✅ 17. نظام الاشتراكات والتراخيص
// ==========================================
const SUBSCRIPTION_API = "https://abogad.vercel.app";
const CLIENT_NAME = "abogad";

async function checkSubscription() {
    try {
        const response = await fetch(`${SUBSCRIPTION_API}/api/codes/check-subscription`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ client_name: CLIENT_NAME })
        });
        
        if (!response.ok) throw new Error('فشل الاتصال بخادم الترخيص');
        
        const data = await response.json();
        const remainingDays = data.remaining_days || 0;
        
        updateSubscriptionUI(remainingDays);
...
document.getElementById('closeWarningBtn')?.addEventListener('click', () => {
    document.getElementById('subscriptionWarningModal').style.display = 'none';
});

// ==========================================
// ✅ 18. الحماية من جهة العميل
// ==========================================
(function() {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        ...
        setupCamera('startProdCameraBtn', 'prodReader', 'prodBarcode', 'autoCloseProdCameraCheckbox', () => document.getElementById('prodName')?.focus());
        setupCamera('startRestockCameraBtn', 'restockReader', 'restockBarcode', 'autoCloseRestockCameraCheckbox', (t) => lookupProductForRestock(t));
    }, 1000);
});