...
// ==========================================
// كاتالوج المنتجات (Single Document) لتقليل استهلاك Firebase
// ==========================================
...
function initFloatingButton() {
    ...
    document.addEventListener('touchmove', (e) => {
        ...
        const size = Math.max(40, Math.min(100, touch.clientX - rect.left));
...
// ==========================================
// 11. التنقل بين شاشة البيع والإدارة
// ==========================================
...
window.editProduct = async function(id) {
    ...
    try {
        ...
        await updateProductCatalog(id, { name, sellPrice: parseFloat(price), barcode: p.barcode, priority: p.priority || 4 });
        alert("تم التعديل");
        loadInventory();
    } catch(e) { alert("خطأ"); }
};
...
async function loadSalesReport(filter = 'all') {
    ...
    invoices.forEach(inv => {
        tbody.innerHTML += `<tr><td>${formatDate(inv.timestamp)}</td><td>${inv.cashier||''}</td><td>${inv.totalSales||0}</td></tr>`;
    });
...
async function loadExpensesReport(filter = 'all') {
    ...
    expenses.forEach(exp => {
        tbody.innerHTML += `<tr><td>${formatDate(exp.date)}</td><td>${exp.title}</td><td>${exp.amount}</td></tr>`;
    });
...
async function loadProfitsReport(filter = 'all') {
    ...
    (await getDocs(collection(db, "invoices"))).forEach(d => {
        const data = d.data();
        if (!start || (data.timestamp >= start && data.timestamp <= end)) { ts += data.totalSales||0; tc += data.totalCost||0; }
    });
    (await getDocs(collection(db, "expenses"))).forEach(d => {
        const data = d.data();
    ...
    const np = ts - tc - te;
    tbody.innerHTML = `<tr><td>${ts.toFixed(2)}</td><td>${tc.toFixed(2)}</td><td style="color:${np>=0?'var(--success)':'var(--danger)'};">${np.toFixed(2)}</td></tr>`;
}
...
async function loadShifts() {
    ...
    if (!shifts.length) tbody.innerHTML = '<tr><td colspan="3">لا توجد ورديات</td></tr>';
}
...
async function loadInvoices() {
    ...
    if (!invoices.length) tbody.innerHTML = '<tr><td colspan="2">لا توجد فواتير</td></tr>';
}
