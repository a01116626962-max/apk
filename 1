...
async function loadStats() {
    ...
    (await getDocs(collection(db, "invoices"))).forEach(d => {
        const data = d.data();
        if (!start || (data.timestamp >= start && data.timestamp <= end)) {
            ts += data.totalSales || 0;
            tc += data.totalCost || 0;
        }
    });
...
async function loadTopSellers(start, end) {
    ...
    try {
        ...
        snap.forEach(doc => {
            const inv = doc.data();
            if ((!start || (inv.timestamp >= start && inv.timestamp <= end)) && inv.items) {
...
window.viewInvoiceDetails = async (id) => {
    ...
    if (snap.exists()) {
        ...
        inv.items.forEach((item, i) => {
            ...
            details += `     ${item.qty} × ${item.price} = ${item.price * item.qty} ج\n`;
        });
        details += `━━━━━━━━━━━━━━━━━\n`;
        details += `💰 الإجمالي: ${inv.totalSales} ج\n`;
        if (inv.totalCost) {
            details += `📊 التكلفة: ${inv.totalCost} ج\n`;
            details += `💎 الربح: ${inv.totalSales - inv.totalCost} ج\n`;
        }
        alert(details);
    }
};
...
function setupCamera(btnId, divId, inputId, checkId, callback) {
    ...
    btn.addEventListener('click', () => {
        ...
        if (open) {
            qrCode.stop().then(() => { div.style.display = 'none'; open = false; btn.innerHTML = '📷'; }).catch(() => { div.style.display = 'none'; open = false; btn.innerHTML = '📷'; });
        } else {
            ...
            ).then(() => { open = true; btn.innerHTML = '❌'; }).catch((err) => { alert("تعذر فتح الكاميرا. تأكد من السماح بالوصول."); div.style.display = 'none'; });
        }
    });
}
